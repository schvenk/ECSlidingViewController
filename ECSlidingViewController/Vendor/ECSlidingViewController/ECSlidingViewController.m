...
@property (nonatomic, unsafe_unretained) BOOL topViewIsMoving;
...
@synthesize topViewIsMoving = _topViewIsMoving;
...
- (void)viewDidLoad
{
    ...
    self.resetTapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(topViewTapped)];
...
- (void)updateTopViewHorizontalCenterWithRecognizer:(UIPanGestureRecognizer *)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        if ((newCenterPosition < self.resettedCenter && self.anchorLeftTopViewCenter == NSNotFound) || (newCenterPosition > self.resettedCenter && self.anchorRightTopViewCenter == NSNotFound)) {
            // User is dragging in a disabled direction
            newCenterPosition = self.resettedCenter;
        } else if (!self.topViewIsMoving) {
            self.topViewIsMoving = YES;
